...
class Gomoku:
    ...
    def make_move(self, row, col): #4 things
        if self.game_over or not self.is_valid_move(row, col):   #1 , 2
            return False

        self.board[row][col] = self.current_player
        self.last_move = (row, col)                 #3
        self.move_history.append((row, col, self.current_player))     #4

        if self.check_winner(row, col):         #5
            self.game_over = True
            self.winner = self.current_player
        elif self.is_board_full():               #6
            self.game_over = True
            self.winner = 0  # Draw

        self.current_player = 3 - self.current_player #good way to altenate between players say that FirstPlayer = 1 and second player =2
        return True

    def undo_move(self):
        if not self.move_history: # if move history is empty
            return False

        row, col, player = self.move_history.pop() # delete the current move to get the step before it
        self.board[row][col] = 0
        self.game_over = False
        self.winner = None
        self.current_player = player
        self.last_move = self.move_history[-1][:2] if self.move_history else None # take the last move and cut two steps from it
        return True

    def is_valid_move(self, row, col): #checks that tha move is not gonna be outside the limits of matrix and doesnt have value
        return 0 <= row < self.size and 0 <= col < self.size and self.board[row][col] == 0

    def is_board_full(self):
        return np.all(self.board != 0)

    def check_winner(self, row, col): # check four directions from the last cell
        player = self.board[row][col]
        directions = [(0, 1), (1, 0), (1, 1), (1, -1)]
        for dr, dc in directions:
            count = 1
            for direction in [1, -1]: #means we move in the direction in directions to the front or to the back
                r, c = row + direction * dr, col + direction * dc #to get column number or row number
    ...
    def get_valid_moves(self):
        if not self.move_history:
            center = self.size // 2
            return [(center, center)] # if there is no history so the best move we can do is from the center

        empty_positions = [(i, j) for i in range(self.size) for j in range(self.size) if self.board[i][j] == 0]

        if len(empty_positions) > 40: #40 is a middle number between the heighest and the lowest
            focused_moves = []
                ...
                for di in range(-2, 3):
                    for dj in range(-2, 3):
                        ...
                        if 0 <= ni < self.size and 0 <= nj < self.size and self.board[ni][nj] != 0:
                            ...
                            break
            if focused_moves: # check if there any moves that is empty around the selected moves (if there is no selected moves it returns the empty position without making periority of any thing)
...
class AIPlayer:
    def __init__(self, player, algorithm="alphabeta", depth=2):
        ...
        self.depth = depth
        self.pattern_weights = { #evaluation ai use it
    ...
    def get_move(self, game):# get the best move for ai based on the following moves
        self.eval_cache = {} #place to save the coming moves
        _, move = (self.minimax(game, self.depth, True) if self.algorithm == "minimax" #the move is gonna move using minimax
...
# it makes like a tree having all the possile moves and get the big one
    def minimax(self, game, depth, maximizing): #it use 2 fuctions (maximizing and minimizing)
        if depth == 0 or game.game_over: #base case
    ...
    def evaluate(self, game):
        if game.game_over:
            if game.winner == self.player: #checks if u r the winner
                return 100000
            elif game.winner == 3 - self.player:#checks if opp r the winner
                return -100000
            return 0
    ...
    def evaluate_line(self, line): #gets the score of each line for u and for the opp but with negative value
        ...
        patterns = {
            ...
            'two': p * 2,
            'opp_five': op * 5,
            'opp_open_four': e + op * 4 + e,
            'opp_four': op * 4,
            'opp_open_three': e + op * 3 + e,
            'opp_three': op * 3,
            'opp_open_two': e + op * 2 + e,
            'opp_two': op * 2
        }
...
class GomokuGUI:
    ...
    def start_game(self):
        ...
        if self.game_mode == 1:  # Human vs AI
            self.ai_player = AIPlayer(2, "minimax", 2)  # AI is always player 2
    ...
    def handle_click(self, pos):
        if self.show_menu:
            ...
            if self.buttons['new_game'].collidepoint(pos):
                ...
                self.show_menu = True
                return  # Return early to avoid processing board clicks

            # Handle board clicks - only human can move in human modes when it's their turn
